...
<head>
    ...
    <script type="text/javascript">
	    $(document).ready(function() {
			$('#phanCong').autocomplete({
				source: '${pageContext.request.contextPath}/admin/searchAuto-phan-cong'
			});
		});
    </script>
</head>
            ...
            <div class="col-sm-10 ">
            	...
            	<nav class="navbar navbar-expand-lg">
                	<ul class="navbar-nav mr-auto">
                		<li>
	                   		<form class="form-inline my-2 my-lg-0" method="get" action="phan-cong" role="search">
	                       		<input id="phanCong" name="k" class="form-control mr-sm-2" type="search" placeholder="Tìm kiếm phân công" aria-label="Search">
                   	...
                   	<ul class="navbar-nav">
                   		<li>   
                   			<c:url value="add-phan-cong" var="addLink">
                ...
                <table class="table table-striped table-hover">
                    <c:choose>
	                    <c:when test="${!empty pagedListHolder.pageList}">
		                    ...
		                    <tbody>
		                    	<c:forEach end="${result}" items="${pagedListHolder.pageList}" var="phanCong" varStatus="loop">
		                    		<c:url var="updateLink" value="edit-phan-cong">
		                    			<c:param name="maPC" value="${phanCong.maPC}"></c:param>
		                    			<c:param name="p" value="${pagedListHolder.page}"></c:param>
		                    			<c:if test="${search}">
		                   					<c:param name="k" value="${k}"></c:param>
		                   				</c:if>
		                    		</c:url>		                    		
		                    		<c:url var="deleteLink" value="delete-phan-cong">
		                    			<c:param name="maPC" value="${phanCong.maPC}"></c:param>
		                    			<c:param name="p" value="${pagedListHolder.page}"></c:param>
		                    			<c:if test="${search}">
		                   					<c:param name="k" value="${k}"></c:param>
